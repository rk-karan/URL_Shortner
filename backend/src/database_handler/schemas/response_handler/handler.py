...
from ..response_schemas.schema import User_Login_Response, User_Create_Response, Payload_Decoded 
from ..response_schemas.schema import User_Profile_Response, Long_URL_Create_Response, Long_URL_Edit_Response
...
def get_login_response(name: str = None, email: str = None, access_token: str = None):
    if not name or not email or not access_token:
        raise Missing_Params
    return make_response(User_Login_Response(user= get_user_details(name= name, email= email), access_token= access_token))
...
def get_user_password_update_response(name: str = None, email: str = None):
    if not name or not email:
        raise Missing_Params
    return make_response(User_Password_Update_Response(user= get_user_details(name= name, email= email)))
